...
public class SimplePaint2 extends Application {
    ...
    private void mousePressed(MouseEvent evt) {
        ...
        /*Here, the point x and y have been added by 0.5 which is the middle point of a pixel.
         *due to the line's width is 1. If not adding 0.5, it will cover the top
         *pixel 0.5 and current pixel 0.5 which will active antialiasing which
         *is not good.
         *
         *If both top and current pixel's color is white and we want to draw a black line(line's width is 1),
         *antialiasing will mix the color white and black to a grey color. So we will get a grey color which 
         *is not we expect. Therefore, adding 0.5 will solve this problem.
         * */
    ...
    private void mouseReleased(MouseEvent evt) {
        ...
        /*If there is only 1 point stored in points' arraylist, then do nothing.
         *This means that the user just press the button and release the button without dragging.
         **/
        if (currentCurve.points.size() > 1)
            curves.add(currentCurve);
        
        /*If an object has no pointer refer to , then it will be discarded automatically by java
         *garbage collection.
         *
         *Therefore, either a currentCurve will be stored in curves' arraylist(has pointer to point it) 
         *or it will be destroyed by the garbage collection.*/
        currentCurve = null;
    }
    ...
    public MenuBar createMenuBar() {
        ...
        for (int i = 0; i < colorNames.length; i++) {
            ...
            item.setUserData(Integer.valueOf(i)); //See the comment below which is above the getUserData()
            									  //It explain the "userData" property.
