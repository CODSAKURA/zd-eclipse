...
public class SimpleColorChooser extends Application {
    ...
    private void newColor(Slider whichSlider) {
        ...
        /*
         *"%02x": x is convert integer to hexadecimal number.
         *		  02 will add 0 in front if there is only one integer; However, "2x" will add a SPACE in front of integer instead of 0.
         *		  For Example, ("02d",3) -> 03 ; ("2d", 3) ->  3.
         *		  "02x" will be needed on the CSS style sheet which is after the sentence "-fx-background-color:".
         *
         *"(int)(255*color.getRed())": Because getRed() return double number from 0 to 1. This means the percentage that red has occupied in 255.
         *							   Therefore, we will need [255 * color.getRed()] to get the real number that red has occupied within 255.
         *							   After that, as "%02x" x is convert INTEGER to hex, so we need to add (int) in front to casting. 
         */
