...
public class Life extends Application {
    ...
    public void start(Stage stage) {
        ...
        /*This if statement is to judge if the cell size is less than 5, then
         *there is no line generated between the two cell.*/
        if (cellSize < 5)
        ...
        timer = new AnimationTimer() {
            ...
            public void handle(long time) {
            	/*Caution: the result of time - previousTime is nanoseconds. 0.975*oneTenthSecond is author's idea, no any logic.
            	 *We could test 1 second draw one frame by using following:
            	 *(time-previousTime)/1_000_000_000.0 > 1
            	 *
            	 *Here if statement is testing:
            	 *1.If the fastCheckbox is selected, then if statement will always true no matter what time difference is.
            	 *2.If the fastCheckbos is unselected, then if statement will test whether the condition of time difference is achieved,
            	 *	if it achieve, then draw the frame. Otherwise, keep looping until the condition achieve.
            	 * */
    ...
    private void mousePressed(MouseEvent e) {
        ...
        if (row >= 0 && row < lifeBoard.getRowCount() && col >= 0 && col < lifeBoard.getColumnCount()) {
        	/*Click secondary mouse will let that specific cell dead.
        	 *Otherwise, clicking other mouse will let that specific cell alive*/
