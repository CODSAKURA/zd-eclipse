...
public class SimplePaint extends Application {
    ...
    public void clearAndDrawPalette() {
        ...
        int colorSpacing = (height - 56) / 7;	//height = 400
        										//56 is: Clear Button + Clear Button's below gap(3 pixels) + the first rectangle's top gap(3 pixels)
        										//So the result is: rectangle area + its below gap(3 pixels)
        ...
        for (int N = 0; N < 7; N++) {
            g.setFill( palette[N] );
            g.fillRect(width-53, 3 + N*colorSpacing, 50, colorSpacing-3);	//3 + N*colorSpacing: 3 is the gap between the window and the first rectangle
            																//50 is width
        }

        /* Draw a 2-pixel white border around the color rectangle of the selected drawing color. 
         * The below code will initialize the black color will color by white border as currentColorNum is 0.
         * If you cover this part, the white border will start appear once you have selected color.
         */

        g.setStroke(Color.WHITE);	//Used to stroke the layout of the object
        g.setLineWidth(2);			//Because the line width is 2, it is the middle point of pen, the pen will draw the top 1 pixel and bottom 1 pixel.
        
        g.strokeRect(width-54, 2 + currentColorNum*colorSpacing, 52, colorSpacing-1);	//54: rectangle's left pixels(1 pixels) + rectangle(50 pixels) + rectangle's right gap(3 pixels)
        																				//2 + currentColorNum*colorSpacing: the height gap to the rectangle is 1 pixels.
        																				//Therefore, the point(width -54 , 2 + currentColorNum*colorSpacing) will lead the top width and height gap to the rectangle is 1 pixels.
        																				//However, this will only make the TOP width and height gap to 1 pixel. BOTTOM width and height gap to rectangle will need to achieve as well.
        
        																				//52: rectangle's left pixels(1 pixels) + rectangle(50 pixels) + rectangle's right pixels(1 pixels)        
        																				//Therefore, the bottom width gap to rectangle has achieve to 1 pixel. 
        
        																				//colorSpacing - 1: Because the point(width -54 , 2 + currentColorNum*colorSpacing) has lead the TOP height gap to the rectangle is 1 pixels, 
        																				//So colorSpacing is [the top height gap to rectangle(1 pixels) + rectangle + original rectangle's bottom gap - 1(3 - 1 = 2 pixels)]
        																				//In order to make the BOTTOM height gap to rectangle is 1 pixels, colorSpacing will need to delete extra 1. NOT DELETE 2!!!!!!
    ...
    private void changeColor(int y) {
        ...
        int newColor = y / colorSpacing;       // Which color number was clicked?
        									   // Divide will not round up like 4.5 = 5. Instead the computer will round down 4.5 = 4.
    ...
    public void mouseDragged(MouseEvent evt) {
        ...
        /*Because the line width is 2, the line will draw 1 pixel in both sides. So in order to not effect the border
         *3 pixels to be 2 pixels,x y below both need to add extra 1 pixels.
         */
        if (x < 4)                          // Adjust the value of x,
            x = 4;                           //   to make sure it's in
        if (x > canvas.getWidth() - 57)       //   the drawing area.
            x = (int)canvas.getWidth() - 57;

        if (y < 4)                          // Adjust the value of y,
