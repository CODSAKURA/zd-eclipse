...
public class RollDice extends Application {
    ...
    private int die1 = (int)(Math.random()*6) + 1;  // The values shown on the dice.
    ...
    private void drawDie(GraphicsContext g, int val, int x, int y) {
        ...
        if (val > 1){
            g.fillOval(x+3, y+3, 9, 9);	// upper left dot
        	g.fillOval(x+23, y+23, 9,9); // bottom right dot
        }        
        if (val % 2 == 1) 
            g.fillOval(x+13, y+13, 9, 9); // middle dot (for odd-numbered val's)
        if (val > 3){
            g.fillOval(x+23, y+3, 9, 9); // upper right dot
        	g.fillOval(x+3, y+23, 9, 9); // bottom left dot
        }
        if (val == 6){
            g.fillOval(x+3, y+13, 9, 9);// middle left dot
        	g.fillOval(x+23, y+13, 9, 9);// middle right dot 
        }    
    }
