...
public class GrowingCircleAnimation extends Application {
    ...
    private void drawFrame(GraphicsContext g, int frameNumber, int width, int height) {
        ...
        for (int i = 0; i < circleData.length; i++) {  // draw the filled circles
            ...
            /*Math.random() < 0.005 is just a condition(no any extra meaning)which means that:
             * If the generated random number is less than 0.005, then no matter
             * how big is the circle stored in that array, it will be replaced by a new circle
             * which radius is 1. Therefore, there are some circle will be replaced even their
             * radiuses have not grow to 100.
             * */
